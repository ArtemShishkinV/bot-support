...
@dp.callback_query_handler(IsRegister(), IsUser(), text='call_support')
async def support_call_from_menu(call: types.CallbackQuery):
    await call.answer()
...
async def answer_support_call(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    if str(await user_state.get_state()) != "wait_in_support":
        user = await db_postgres.get_user_by_id(call.from_user.id)
        await call.message.edit_text("К сожалению, пользователь уже передумал.", reply_markup=await menu_keyboard(user))
        return
...
async def exit_support(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    if await second_state.get_state() is not None:
        ...
        if int(second_id) == call.from_user.id:
            ...
            await bot.send_message(user_id, f'Пользователь завершил сеанс техподдержки\n'
                                            f'Для продолжения вызовите /menu')

    await call.message.edit_text(f'Вы завершили сеанс\n'
                                 f'Для продолжения вызовите /menu')
    await state.reset_state()
